...
class StudentRegistrationForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()
            # Don't create UserProfile here - let the signal handler do it
            # But if signal fails for some reason, use get_or_create as backup
            profile, created = UserProfile.objects.get_or_create(
                user=user,
                defaults={
                    'role': 'student',
                    'student_id': None  # Will be generated by signal or below
                }
            )
            # If profile already exists but doesn't have student_id, add it
